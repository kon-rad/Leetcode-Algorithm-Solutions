        ...
        switch (q[0]) {
            case 1:
                if (d.hasOwnProperty(q[1])) {
                    decrementIfExists(freq, d[q[1]]);
                    d[q[1]]++;
                    incrementIfExists(freq, d[q[1]]);
                } else {
                    d[q[1]] = 1;
                    incrementIfExists(freq, d[q[1]]);
                }
                break;
            case 2:
                if (d.hasOwnProperty(q[1]) && d[q[1]] > 0) {
                    decrementIfExists(freq, d[q[1]]);
                    decrementIfExists(d, q[1]);
                    incrementIfExists(freq, d[q[1]]);
                }
                break;
            case 3:
                if (freq.hasOwnProperty(q[1]) && freq[q[1]] > 0) {
                    r.push(1);
                    return;
                }
                r.push(0);
...
function decrementIfExists(d, n) {
    if (d.hasOwnProperty(n)) {
        d[n] = d[n] - 1 < 0 ? 0 : d[n] - 1;
    }
}
function incrementIfExists(d, n) {
    if (d.hasOwnProperty(n)) {
        d[n]++;
        return;
    } 
